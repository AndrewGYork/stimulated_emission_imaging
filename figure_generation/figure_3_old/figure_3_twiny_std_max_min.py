...
def main():
    ...
    std_filename = (
        './../../stimulated_emission_data/figure_3/dataset_registered_std.tif')
    std = np_tif.tif_to_array(std_filename).astype(np.float64)
    maxp_filename = (
        './../../stimulated_emission_data/figure_3/dataset_registered_max.tif')
    maxp = np_tif.tif_to_array(maxp_filename).astype(np.float64)
    minp_filename = (
        './../../stimulated_emission_data/figure_3/dataset_registered_min.tif')
    minp = np_tif.tif_to_array(minp_filename).astype(np.float64)
...
##        data[:,:,0,:,:] # max red/green delay
    ...
    maxp_fluorescence_stack = 0.5 * (maxp[:,:,0,:,:] + std[:,:,4,:,:])
    maxp_depleted_stack = maxp[:,:,2,:,:] # zero red/green delay
    minp_fluorescence_stack = 0.5 * (minp[:,:,0,:,:] + std[:,:,4,:,:])
    minp_depleted_stack = minp[:,:,2,:,:] # zero red/green delay
...
##        depleted_stack.reshape(tif_shape),'depleted_stack_ctrl.tif')
    ...
    maxp_fluorescence_signal_bg = (
        maxp_fluorescence_stack[:,:,top_bg:bot_bg,left_bg:right_bg
                           ].mean(axis=3).mean(axis=2)
        )
    minp_depleted_signal_bg = (
        minp_depleted_stack[:,:,top_bg:bot_bg,left_bg:right_bg
                       ].mean(axis=3).mean(axis=2)
        )
    maxp_fluorescence_signal_bg = (
        minp_fluorescence_stack[:,:,top_bg:bot_bg,left_bg:right_bg
                           ].mean(axis=3).mean(axis=2)
        )
    minp_depleted_signal_bg = (
        minp_depleted_stack[:,:,top_bg:bot_bg,left_bg:right_bg
...
##    std_fluorescence_signal = std_fluorescence_signal - std_fluorescence_signal_bg
    maxp_depleted_signal = (
        maxp_depleted_stack[:,:,top:bot,left:right].mean(axis=3).mean(axis=2))
    maxp_depleted_signal = maxp_depleted_signal - maxp_depleted_signal_bg
    maxp_fluorescence_signal = (
        maxp_fluorescence_stack[:,:,top:bot,left:right].mean(axis=3).mean(axis=2))
    maxp_fluorescence_signal = maxp_fluorescence_signal - maxp_fluorescence_signal_bg
